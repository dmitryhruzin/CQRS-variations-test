...
import { UserEnteredTheSystemEventHandler } from './UserEnteredTheSystemEventHandler.js'
...
describe('UserEnteredTheSystemEventHandler', () => {
  describe('handle', () => {
    let repository: UserMainRepository
    let handler: UserEnteredTheSystemEventHandler
    ...
    beforeEach(() => {
      ...
      handler = new UserEnteredTheSystemEventHandler(repository)
    })

    const testCases = [
      {
        description: 'should call repository with specific event',
        payload: new UserEnteredTheSystemV1({ aggregateId: '1', aggregateVersion: 2 }),
        expectedArgument1: '1',
        expectedArgument2: { version: 2, active: true }
      }
    ]

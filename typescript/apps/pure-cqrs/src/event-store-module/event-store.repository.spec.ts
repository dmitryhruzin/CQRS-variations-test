...
describe('EventStoreRepository', () => {
  ...
  describe('saveEvents', () => {
    const EVENTS_MOCK: Event[] = [
      { aggregateId: '123', aggregateVersion: 1, toJson: () => JSON.stringify({ name: 'John Doe' }) },
      { aggregateId: '123', aggregateVersion: 2, toJson: () => JSON.stringify({ name: 'John Smith' }) }
    ]
    ...
    const testCases = [
      {
        ...
        expected: true,
        saved: [
          { aggregateId: '4', body: JSON.stringify({ name: 'John Doe' }), aggregateVersion: 1 },
          { aggregateId: '4', body: JSON.stringify({ name: 'John Smith' }), aggregateVersion: 2 }
        ]
      },
      {
